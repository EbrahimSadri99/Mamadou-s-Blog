<template>
  <div class="relative">
    ...
    >
      ...
      <div
        class="absolute inset-0 pattern-islamic-star opacity-[0.03] dark:opacity-[0.05]"
      ></div>
        ...
        <div class="max-w-4xl mx-auto">
          ...
          <div class="flex items-center gap-3 mb-6 animate-fade-in">
            <div class="flex gap-1">
              <div
                class="w-2 h-2 bg-emerald-500 dark:bg-emerald-400 rotate-45"
              ></div>
              <div
                class="w-2 h-2 bg-teal-500 dark:bg-teal-400 rounded-full"
              ></div>
            </div>
          ...
          <!-- Metadata -->
          <div
            class="flex flex-wrap gap-6 text-slate-600 dark:text-slate-400 font-lora animate-fade-in"
          ...
          <!-- Decorative divider -->
          <div
            class="flex items-center gap-4 my-12 animate-fade-in"
    ...
    <main class="container mx-auto px-4 py-16">
      <div v-if="post" class="max-w-4xl mx-auto">
        ...
        >
          <div class="flex items-center gap-3 mb-4">
            ...
            >
              <svg
                class="w-4 h-4 text-white -rotate-45"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <h3
              class="font-cormorant text-2xl font-bold text-slate-800 dark:text-slate-200"
            >
              Topics
            </h3>
          </div>
      ...
      <div v-else class="max-w-2xl mx-auto text-center py-20">
        ...
        <h2
          class="font-cormorant text-4xl font-bold mb-4 text-slate-900 dark:text-slate-100"
        >
          Article Not Found
        </h2>
...
const loadPost = async () => {
  try {
    ...
    // Get single row from MainDB by source_id
    ...
    // Public Google Drive raw HTML endpoint
    const githubURL = `https://raw.githubusercontent.com/EbrahimSadri/MamadouBlogDatabase/refs/heads/main/${postId}.html`
    ...
    let html_str = doc.body.innerHTML
    const cleanedHtml = removeInlineStyles(html_str)
    htmlContent.value = cleanedHtml

  } catch (error) {
    console.error('Error loading post:', error)
    htmlContent.value = `<h2>ERROR POST NOT FOUND</h2>`
  }
}
  ...
  meta: [
    {
      name: "description",
      content: post.value
        ? `Read ${post.value.title} | Mamadou Dialo\'s Blog`
